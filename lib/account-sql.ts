...
/**
 * Turn sharing on. The token is generated by the application with
 * `crypto.randomBytes` and written once: `where share_token is null` means
 * pressing Share twice does not mint a second address for the same list and
 * quietly invalidate the one already in somebody's chat window.
 */
export const SHARE_COLLECTION_SQL = `
  update public.collections
     set share_token = $2::text, is_public = true
